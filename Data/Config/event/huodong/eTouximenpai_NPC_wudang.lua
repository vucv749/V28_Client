--MisDescBegin
--k¸ch bän g¯c Hào
x808033_g_ScriptId	= 808033
--bän sao tên
x808033_g_CopySceneName	= "Võ Ðang S½n"
--nhi®m vø Hào
x808033_g_MissionId			= 1250
--Thßþng mµt cái nhi®m vø Ðích ID
x808033_g_MissionIdPre	= 0
--møc tiêu NPC
x808033_g_Name					= "Ðánh Lén Môn Phái"
--hay không Th¸ tinh anh nhi®m vø
x808033_g_IfMissionElite= 1
--nhi®m vø c¤p b§c
x808033_g_MissionLevel	= 10000
--nhi®m vø phân loÕi
x808033_g_MissionKind		= 1
--nhi®m vø vån bän miêu tä
x808033_g_MissionName			= "Ðánh Lén Môn Phái"
--nhi®m vø miêu tä
x808033_g_MissionInfo			= "  "
--nhi®m vø møc tiêu
x808033_g_MissionTarget		= "  Di®t hªt quái v§t li«n có th¬ hoàn t¤t nhi®m vø"
--chßa xong Thành nhi®m vø Ðích Npc ð¯i thoÕi
x808033_g_ContinueInfo		= "  "
--hoàn thành nhi®m vø Npc nói chuy®n trong l¶i nói
x808033_g_MissionComplete	= "  "

--vai di­n sinh ðµng Tr¸ bän sao Ðích index
x808033_g_activePointIndex = 3

--********phía dß¾i Ki HÕng Th¸ ðµng thái bi¬u hi®n Ðích nµi dung, dùng cho TÕi nhi®m vø Li®t Bi¬u Trung ðµng thái bi¬u hi®n nhi®m vø tình hu¯ng******
--tu¥n hoàn nhi®m vø Ðích s¯ li®u hß¾ng dçn tra cÑu, bên trong T°n Trß¾c Dî T¯ Ðích Hoàn S± MD_SHUILAO_HUAN
--nhi®m vø hay không ðã hoàn thành
--**********************************ðã ngoài Th¸ ðµng thái****************************
--vai di­n Missionlßþng biªn ð±i thuyªt minh
x808033_g_Param_IsMissionOkFail	= 0						--0?:????????(0???;1??)
x808033_g_Param_killmonstercount	= 1					--1?:?????????
x808033_g_Param_killbosscount	= 2							--2?:????boss????
x808033_g_Param_sceneid		= 3									--3?:??????????
x808033_g_Param_teamid		= 4									--4?:???????????
x808033_g_Param_time			= 5									--5?:????????(??:?)
--6Hào: Cø th¬ bän sao sñ ki®n k¸ch bän g¯c chiªm døng
--7Hào: Cø th¬ bän sao sñ ki®n k¸ch bän g¯c chiªm døng

--MisDescEnd
