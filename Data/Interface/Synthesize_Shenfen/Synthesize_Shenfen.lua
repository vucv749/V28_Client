...
function Synthesize_Shenfen_ListBox_Selected()
	...
	if table.getn(tabMaterials) > 0 then 
		...
		if Prescr_Ability == 61 then 
			-- ²ú³öIdÏÔÊ¾³ÉItemCompoundHelper.txtÅäµÄ²ú³öId
	...
	if resultid == -1 then 
		...
		if Prescr_Ability == 60 then 
			-- ÌØÐ´£ºÉí·ÝÖÆÒ©²ú³öÊýÁ¿ÊÇ¸ÅÂÊËæ»úµÄ£¬¹ÊÒþ²Ø
			Synthesize_Shenfen_Amount : Hide();
		end
	...
	-- 	Synthesize_Shenfen_Item : SetToolTip(tip_name.."#rÀàÐÍ£º"..tip_type..strName);
	-- 	Synthesize_Shenfen_Amount:SetToolTip(tip_name.."#rÀàÐÍ£º"..tip_type..strName);
	-- end
