	...
	if u_index == 1 then
		if theAction:GetID() ~= 0 then
				...
				if Original_Visual_ID < 10000 then
					PushDebugMessage("Trang b¸ này không th¬ ð±i ngoÕi hình")
					EquipChange_Resume_Gem(23)
					return
	...
	elseif u_index == 2 then
		if theAction:GetID() ~= 0 then
				...
				if PlayerPackage : GetItemTableIndex( i_index ) ~= 30900004 then
					PushDebugMessage("Chï có th¬ ð£t Biªn Hình Phù vào.")
					return
				end
...
function EquipChange_Buttons_Clicked()
	...
	if Change_Item1 == -1 then
		PushDebugMessage("Hãy ð£t trang b¸ c¥n ð±i ngoÕi hình vào!")
		return
	end
