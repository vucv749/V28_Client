...
local g_GemTableIndex = -1			--宝石背包索引
local g_ReplaceTableIndex = 20800022--无相宝石
local g_ComposeBagIndex = -1		--合成符背包索引
local g_ReplaceBagIndex = -1		--替代符背包索引
...
function GemMerge_OnLoad()
	...
	g_ActionButton_PackageGem = GemMerge_GemItem1 		--背包中需要升级宝石
	g_ActionButton_ReplaceItem = GemMerge_GemItem2 		--宝石替代符
	g_ActionButton_ReplaceItem : SetDrawCorner(3, false);
	g_ActionButton_ProductItem = GemMerge_ProductItem 	--产出的下级宝石
	g_ActionButton_ComposeItem = GemMerge_NeedItem		--宝石合成符
...
function GemMerge_OnEvent(event)
	...
	if event == "UI_COMMAND" and tonumber(arg0) == 2024061401 then	--无相宝石合成宝石
		GemMerge_Clear()
...
function GemMerge_PutInGem(bagPos)
	...
	if nGemLevel ~= 5 then
		PushDebugMessage("#{BSFHC_240613_17}")		--只能放入5级宝石（不含冥石）
		return
	end
...
function GemMerge_PutInComposeItem(bagPos)
	...
	elseif GemMerge_IsComposeItem(bagPos) ~= 1 then
		-- PushDebugMessage("#{BSQHB_120830_03}")		--材料类型不符
		PushDebugMessage("#{BSFHC_240613_18}")		--请放入高级宝石合成符
		return
	end
	...
	if GemMerge_GetSpecialMaterial() == -1 then
		PushDebugMessage("RuleTable不支持合成当前等级的宝石")
		return	
...
function GemMerge_RecalcSuccOdds()
	...
	elseif PlayerPackage:CountAvailableItemByIDTable(g_ReplaceTableIndex) <= 0 then 
		GemMerge_GaiLvNum:SetText("#{BSFHC_240613_09}") --无相宝石数量不足
	else
		local nOdds = 75
