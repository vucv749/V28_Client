...
local g_PointLevel = 0 --玩家当前的阶位
local g_IsShowRedPoint = 0

local nFullLevel = 0 --是否满分了

local g_LevelPrizeinfo = {}

local g_EachLeveActivePoint = 10

local g_MAX_MISSION_NUM = 5 --最大任务
...
function ShengWang_Pass_OnEvent(event)
	...
	elseif event == "UI_COMMAND" and (tonumber(arg0) == 89021501) then	--服务端传数据
	
		local nOper = Get_XParam_INT(0)	--界面操作类型（0 关闭，1打开， 2刷新）
		local nTime  = Get_XParam_INT(1) --服务器当前天数时间
		g_MonthActivePoint = Get_XParam_INT(2) --玩家当月战令点数
		g_TodayGetPoint = Get_XParam_INT(3) --玩家当日战令点数
		g_IsAroused = Get_XParam_INT(6) --是否激活RMB1
		g_IsShowRedPoint = Get_XParam_INT(7) --是否显示红点
		g_IsAroused2 = Get_XParam_INT(9) --是否激活RMB2
		...
		elseif 2 == nOper then
			...
			-- 只有打开页面时刷新页面才记录上一次的滚动条
			g_Position_Scroll = ShengWang_Pass_RewardList:GetScrollPosition()	--保存上一次滚动条位置
		end
	...
	elseif event == "UI_COMMAND" and (tonumber(arg0) == 89021505) then	--服务端传数据
	
		local nMissionData = Get_XParam_INT(0)	--任务数据
		local nMissionGetAwardData  = Get_XParam_INT(1) --任务领取奖励数据
...
function ShengWang_Pass_GetPrizeInfo()
		...
		local prizeB_MD = Get_XParam_INT(4) --普通奖励MD
		local prizeA_MD = Get_XParam_INT(5) --付费1奖励MD
		local prizeC_MD = Get_XParam_INT(8) --付费2奖励MD
...
function ShengWang_Pass_PrizePageOpen()	
	if 1 ~= ShengWang_Pass_GetPrizeInfo() then  --加载奖励表信息
		PushDebugMessage("页面数据加载失败，请重新打开页面。")
		return
	end
...
function ShengWang_Pass_Arouse()
	Clear_XSCRIPT()
			...
			Set_XSCRIPT_ScriptID(890215) --脚本编号 
...
function ShengWang_Pass_GetMissionAward(index)
	-- Clear_XSCRIPT()
			-- Set_XSCRIPT_Function_Name("AskMissionAward")  --脚本接口
			-- Set_XSCRIPT_ScriptID(890215) --脚本编号 
