--ÐÂ¹¹¼Ü:ÎÞÐè¸Ä¶¯UI×ÔÊÊÓ¦server¸üÐÂÄ£Ê½
--Ã¿È ºÀÀñ
--´úÂëÐÅÌõ£ºÈ«Ãæ£¬¼ò½à£¬È«¶¼Òª
--***************************************************
	...
	for i=1,8 do
		if RankListInfo[i][2] ~= 0 then--????? ??0?????????
			...
			if RankListInfo[i][7] == 1 then--????0???1???2???
				...
				nTempText = string.format("#GnHu c¥u ðªm: %s/%s",tostring(MuDingPayRewardPlayerPoint),tostring(RankListInfo[i][2]))
			elseif RankListInfo[i][7] == 2 then
				...
				nTempText = string.format("#cff0000nhu c¥u ðªm: %s/%s",tostring(MuDingPayRewardPlayerPoint),tostring(RankListInfo[i][2]))
			end
...
function PayReward_SelectClickTwo(index)
	if PayReward_SelectIndex == 0 then
		PushDebugMessage("Thïnh Tiên Câu Tuy¬n phía trên Ðích v¸ trí lña ch÷n Khuông")
		return
...
function PayReward_SubmitSecect()
	...
	--ÏÂÃæ·¢ËÍ¸ø·þÎñ¶Ë ´æÆðÀ´ Íæ¼Ò´óÍËÔÙ´ò¿ªµÄÊ±ºò¿´µ½µÄÒÀÈ»ÊÇÑ¡ÔñºÃµÄÎïÆ·
	Clear_XSCRIPT() 
