...
function Pet_Show_PetInfo(nIndex)
	...
	elseif (nGeneration >= 100) then
		...
		if (nPetType == 0) then
			--0:宝宝 2023 修改 由记录 上次繁殖等级 改成记录 已经繁殖次数
	...
	Pet_Blood:SetText("血:"..tostring(iHP) .."/".. tostring(iMaxHP))
...
function Pet_Possession_Clicked()
	-- 已经提交到指定界面容器的珍兽不能出战
	if (IsWindowShow("PetSavvy") and Pet:GetPetLocation(PETNUM) == 12)			-- 用成年珍兽提升珍兽悟性 PET_LOCATION_SAVVY
		or (IsWindowShow("PetSavvyGGD")	and Pet:GetPetLocation(PETNUM) == 3)	-- 用根骨丹提升珍兽悟性 PET_LOCATION_SAVVYGGD
		or (IsWindowShow("MissionReply") and Pet:GetPetLocation(PETNUM) == 7)	-- 任务提交物品或珍兽 PET_LOCATION_MISSIONREPLY
		or (IsWindowShow("PetHuanhua") and Pet:GetPetLocation(PETNUM) == 5)		-- 珍兽在幻化，位置为5  PET_LOCATION_HUANHUA
		then
		-- 珍兽处于提交状态，无法附体。
