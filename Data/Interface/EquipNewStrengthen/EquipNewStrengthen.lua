...
function EquipNewStrengthen_OnEvent(event)
	...
	elseif (event == "TAKE_STENGTHEN_ITEM" and this:IsVisible()) then
		if arg0 == "I12" then		--移除左边,要连同右边一起移除
			EquipNewStrengthen_Clear(0)
		elseif arg0 == "I13" then		--移除右边,不需要理会左边
			EquipNewStrengthen_Clear(1)
		end
...
function EquipNewStrengthen_Update(boxpos,Item_index)
	...
	if tonumber(boxpos) == 0 then
		...
		if PlayerPackage:IsBagItemKFS(i_index) ==1 then  --武魂的限制
			PushDebugMessage("#{ZBQH_130521_11}")
			return
		end
		
		if EquipPoint >= 19 and EquipPoint <= 24 then  --灵武的限制
	...
	if tonumber(boxpos) == 1 then
		...
		if EquipPoint2 >= 19 and EquipPoint2 <= 24 then  --灵武的限制
			PushDebugMessage("#{SZXT_221216_213}")
			return
		end
...
function EquipNewStrengthen_Buttons_Clicked()
	...
	if PlayerPackage:IsBagItemKFS(Enchange_Item1) ==1 then  --武魂的限制
		PushDebugMessage("#{ZBQH_130521_15}")
		return
	end
	
	if EquipPoint1 >= 19 and EquipPoint1 <= 24 then  --灵武的限制
		PushDebugMessage("#{SZXT_221216_213}")
		return
	end
	...
	if PlayerPackage:IsBagItemKFS(Enchange_Item2) ==1 then  --武魂的限制
		PushDebugMessage("#{ZBQH_130521_15}")
		return
	end
