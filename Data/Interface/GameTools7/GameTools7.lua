--珍兽修改器 V7
--适配苍山雪复古 雪舞 @WAYLEE 2024-2-16 14:25:21

local g_GameTools7_Frame_UnifiedPosition;
local g_nSelect_Index = -1; --选中的宝宝
local XingGeList = {"胆小","谨慎","忠诚","精明","勇敢","预留1","预留2","预留3","预留4","预留5"}
local StarId = -1
...
function GameTools7_XingGe_ListBox_Selected()
	...
	StarId = StarId - 1
	Pet_AskExtraData(StarId) --服务端数据刷新到客户端
end
...
function GameTools7_OnEvent(event)
	...
	elseif ( event=="UI_COMMAND" and tonumber(arg0) == 202402161 ) and this : IsVisible() then
		...
		--基础资质
		strName = Pet:GetStrAptitude(g_nSelect_Index) --力量
		GameTools7_LiLiangEdix:SetText(strName );
		strName = Pet:GetIntAptitude(g_nSelect_Index) --灵气
		GameTools7_LingQiEdix:SetText(strName );
		strName = Pet:GetPFAptitude(g_nSelect_Index)  --体力
		GameTools7_TiLiEdix:SetText(strName );
		strName = Pet:GetStaAptitude(g_nSelect_Index) --定力
		GameTools7_DingLiEdix:SetText(strName );
		strName = Pet:GetDexAptitude(g_nSelect_Index) --身法
		...
		--品种
		GameTools7_XinXiTxt2:SetText("#Y品种:#G"..strName2);
		
		--珍兽GUID显示
		local petGUID_H,petGUID_L,sex = Pet:GetID(g_nSelect_Index);
		
		GameTools7_XinXiTxt3:SetText("#YGUID高位:#G"..petGUID_H);
		GameTools7_XinXiTxt4:SetText("#YGUID低位:#G"..petGUID_L);
		if(sex == 1) then
			strName = "雄性";
		else
			strName = "雌性";
		end
		GameTools7_XinXiTxt1:SetText("#Y珍兽性别:#G"..strName);
		--上次繁殖等级
		...
		GameTools7_ExpTxt2:SetText("经验上限:"..strName2);
		...
		if PlayerPackage:IsPetLock(g_nSelect_Index) == 1 then
			...
			if( nUnlockElapsedTime ==0) then
				...
				GameTools7_Pet_lock : SetToolTip ("已加锁");
			else
		...
		for i=1,12 do
			...
			PETSKILL_SKILL_EDIX[i]:SetText(theSkillAction:GetDefineID()) 	--填写技能ID
			PETSKILL_SKILL_NAME[i]:SetText(theSkillAction:GetName())		--填写技能命中
		end
...
function GameTools7_DuQu_Clicked()
	if g_nSelect_Index == -1 then
		PushDebugMessage("请先选择需要读取信息的宝宝")
		return
	end
	Clear_XSCRIPT()
...
function GameTools7_PetGrow_Clicked()
	...
	--用于效验数值，拆分十六进制
	local Hex = LuaFnFloatToHex(nNum/1000)  --IEEE754转换
	local part1 = string.sub(Hex, 1, 4) -- 提取前4个字符
	local part2 = string.sub(Hex, 5)    -- 从第5个字符开始提取直到字符串末尾
	Clear_XSCRIPT()
		Set_XSCRIPT_Function_Name("ModifyPet")
		Set_XSCRIPT_ScriptID(666661)
...
function GameTools7_ChengZhangLv_Clicked()
	...
	local nNum1 = tonumber(GameTools7_ChengZhangLvButton:GetCheck())  -- 0 或者 1
	local nNum2 = tonumber(GameTools7_SuoDingButton2:GetCheck())  -- 0 或者 1
	local nNum = 0
