...
function MiniMap_PreLoad()
	...
	this:RegisterEvent("XINCHUNYINGJING_FLEX_UPDATE")-- 移植-新春签到活动-天禧春华战江湖
...
function MiniMap_OnLoad()
	...
	MiniMap_HeXinChun:Hide()-- 移植-新春签到活动-天禧春华战江湖
	MiniMap_ZhenShouYuRe:Hide()-- 2022珍兽预热活动
	MiniMap_ThreeDayFeedBack:Hide() --版本三日登陆 2022Q3 周年庆
...
--		PushDebugMessage("aa "..IsShowButton.." "..IsShowTips)
	...
	elseif ( event == "UI_COMMAND" and tonumber(arg0) == 89306901) then-- 2022珍兽预热活动
		local IsShowButton = Get_XParam_INT(0)
		local IsShowTips = Get_XParam_INT(1)
		-- PushDebugMessage("aa "..IsShowButton.." "..IsShowTips)
		if IsShowButton==1 then
...
function MiniMap_PlayerAsk_Bn_Clicked()
	...
	urlStr = urlStr..",Profession:"..strName	--门派
	urlStr = urlStr..",gameVersion:"..GetGameVersion();	
	urlStr = urlStr.."}";
	
	local vcode = CalciDiaoYanVCode(urlStr.."345341234567353534/Vo48a31bd=");   --后面是SECRET_KEY
...
--	MiniMap_XinShouNew:PlayWarning( 0 );
	Clear_XSCRIPT()
		Set_XSCRIPT_Function_Name( "OnOpenXinShouNewWindow" ); 		-- 脚本号
		Set_XSCRIPT_ScriptID( 892679 );						-- 脚本编号
...
function MiniMap_LandRewardBtnClk()
	...
	Clear_XSCRIPT()
		Set_XSCRIPT_Function_Name( "OpenPrizeUI" ); 		-- 脚本号
		Set_XSCRIPT_ScriptID( 892678 );						-- 脚本编号
		Set_XSCRIPT_ParamCount( 0 );						-- 参数个数
	Send_XSCRIPT()
end
...
function MiniMap_FeelFeedBack800BtnClk()
	...
	Clear_XSCRIPT()
		...
		Set_XSCRIPT_ScriptID( 792012 );						-- 脚本编号
		Set_XSCRIPT_ParamCount( 0 );						-- 参数个数
	Send_XSCRIPT()
end
...
function MiniMap_ZLHeroMeetingBtnClk()
	...
	Set_XSCRIPT_ScriptID( 891119 );						-- 脚本编号
	Set_XSCRIPT_ParamCount( 0 );						-- 参数个数
	Send_XSCRIPT()
end
...
function MiniMap_FenxianClicked()
	...
	-- PushDebugMessage("快捷传送功能暂未开放")
	local curSceneID = GetSceneID();
	if curSceneID > 2 then
		PushDebugMessage( "万能传送只能在主城内使用!" )
		return
	end
