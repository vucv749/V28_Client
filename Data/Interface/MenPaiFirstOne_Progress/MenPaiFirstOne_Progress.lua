...
local MenPaiFirstOne_Progress_Battle_OneTimeBegin = 10090  --MenPaiFirstOne_Progress.lua 里面有一样的宏
...
local MenPaiFirstOne_Progress_Battle_FourTimeEnd = 11470--MenPaiFirstOne_Progress.lua 里面有一样的宏
...
function MenPaiFirstOne_Progress_OnEvent(event)
	...
	if (event=="SCENE_TRANSED") then
		...
		if arg0=="Hjyanmenguan_PVP" then --进战场就开这个界面
...
function MenPaiFirstOne_Progress_Battle_Fresh()	
	...
	local unitId= CMenPaiDiYiData:GetUnitId() --unitId=0代表16强 unitId=1代表8强 unitId=2代表4强 unitId=3代表决赛
	local tips
	if unitId == 0 then
