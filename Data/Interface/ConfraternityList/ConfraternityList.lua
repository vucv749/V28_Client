...
function Guild_List_Detail_Change( idx )
	...
	g_GuildListCtl.foundtime:SetText("Th‡nh lßp: "..szInfo);
	...
	g_GuildListCtl.count:SetText("Ngﬂ∂i: "..szInfo);
	
	local szInfo = Guild:GetGuildInfo(idx, "CityName");
	if(nil == szInfo or 0 == string.len(szInfo)) then
	g_GuildListCtl.city:SetText("Th‡nh: ".."KhÙng cÛ th‡nh");
	else
		g_GuildListCtl.city:SetText("Th‡nh: "..szInfo);
	end
...
function Guild_List_Join()
	...
	if( -1 == Player:GetData("GUILD")) then
		...
		PushDebugMessage("–„ l‡ "..szGuildName.."th‡nh viÍn, r∂i bang trﬂæc r∞i mæi xin l’i.");
		return;
	end
