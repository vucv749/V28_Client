...
local YbMarket_ItemCount_Show = 0 		-- 当前显示的物品数
local YbMarket_ItemIndex_Show = {}		-- 显示的物品下标
local YbMarket_ItemCount_Select = 0 	-- 当前选中的物品数
local YbMarket_ItemIndex_Select = {} 	-- 选中的物品下标
...
function YbMarket_OnLoad()
	...
	YbMarket_EquipType1:AddTextItem("#{SBFW_20230707_304}", 13)	--七情刃 aka 神兵
...
function OnYbMarket_AllBKClicked(index)
	...
	if YbMarket_nSelectTable == 2 then--物品特写，后续代码不执行
...
function OnYbMarket_btnBuyClicked()
	...
	if YbMarket_nSelectTable == 2 then--物品特写，后续代码不执行
		...
		if YbMarket_ItemCount_Select <= 0 then
			PushDebugMessage("#{SHPLGM_150428_05}")--请选择您要购买的物品
			return
		end		
		--安全时间
		if ( tonumber( DataPool:GetLeftProtectTime() ) > 0 ) then
		...
		if YbMarket_ItemCount_Select > 10 then
			PushDebugMessage("#{SHPLGM_150428_11}")--无法购买，每次最多只能购买10项物品
			return
		end	
		--只购买一件物品
...
function YbMarket_ItemCheck(index)
	...
	if bCheck == 1 then--已经选中的取消选中
		--YbMarket_ctrlItemFlash[index]:Hide()
		YbMarket_ctrlItemChecks[index] : SetCheck(0)--取消选中
		YbMarket_ItemIndex_Select[index] = 0
		YbMarket_ItemCount_Select = YbMarket_ItemCount_Select - 1--数量自减
		if YbMarket_ItemCount_Select ~= YbMarket_ItemCount_Show then--不满足勾选上限
			YbMarket_DX:SetCheck(0)--取消全选
		end
	else	--未选中的进入勾选逻辑
		if YbMarket_ItemCount_Select >= 10 then--超过勾选上限
			PushDebugMessage("#{SHPLGM_150428_01}")--勾选已达上限，每次最多可勾选10项物品。
			return
		else--可以选中
			--YbMarket_ctrlItemFlash[index]:Show()
			YbMarket_ctrlItemChecks[index] : SetCheck(1)--选中
			YbMarket_ItemIndex_Select[index] = 1
			YbMarket_ItemCount_Select = YbMarket_ItemCount_Select + 1--数量自增
			if YbMarket_ItemCount_Select == YbMarket_ItemCount_Show then--达到勾选上限
				YbMarket_DX:SetCheck(1)--选中全选
			end
		end
	end
end
...
function OnYbMarket_ItemCheckAll()
	...
	if bCheck == 1 then--已经选中的取消选中
		--YbMarket_DX : SetCheck(0)--取消选中
		YbMarket_ItemCount_Select = 0--数量清空		
		for i=1,10 do--全部取消选中
			if YbMarket_ItemIndex_Show[i] == 1 and YbMarket_ItemIndex_Select[i] == 1 then
				--YbMarket_ctrlItemFlash[i] : Hide()
				YbMarket_ctrlItemChecks[i] : SetCheck(0)--选中
				YbMarket_ItemIndex_Select[i] = 0
			end
		end
	else	--未选中的进入勾选逻辑
		if YbMarket_ItemCount_Show > 10 then--超过勾选上限
			PushDebugMessage("#{SHPLGM_150428_01}")--勾选已达上限，每次最多可勾选10项物品。
			return
		else--可以选中
			--YbMarket_DX : SetCheck(1)--选中全选
			YbMarket_ItemCount_Select = YbMarket_ItemCount_Show--数量设置
			for i=1,10 do--全部选中
				if YbMarket_ItemIndex_Show[i] == 1 and YbMarket_ItemIndex_Select[i] ~= 1 then
					--YbMarket_ctrlItemFlash[i] : Show()
					YbMarket_ctrlItemChecks[i] : SetCheck(1)--选中
