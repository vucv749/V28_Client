...
function TargetPeak_OnEvent(event)
	...
	if event == "UI_COMMAND" and tonumber(arg0) == 20251127 then
		...
		-- if not CachedTarget:CanGetTargetEquip() then
			-- PushDebugMessage("距离该玩家太远，无法查看资料。")				-- 距离该玩家太远，无法查看资料。
			-- return
		-- end
...
function TargetPeak_Update()
	...
	for i=1,3 do
		...
		TargetPeak_WuxueLearn_Attack[i]:SetText("#cff0000尚未选择门派")
		...
		TargetPeak_WuxueLearn_Defence[i]:SetText("#cff0000尚未选择门派")
	...
	for i=1,3 do
		if AttackMenPai[i] > 0 then
			...
			TargetPeak_WuxueLearn_Attack[i]:SetToolTip("#cfff263最多可对#G"..szJinGongMenPai.."#cfff263门派造成的伤害增加#G"..tostring(nAttackMenPaiXDD_ex).."#cfff263（需计算对方的守御武诀）")
		...
		if DefenceMenPai[i] > 0 then
			...
			TargetPeak_WuxueLearn_Defence[i]:SetToolTip("#G"..szFangYuMenPai.."#cfff263门派对自身造成的伤害最多降低#G"..tostring( nDefenceMenPaiXDD_ex).."#cfff263（需计算对方的攻击武诀）")
