...
local bIsPacketOpened = 0 --背包状态（0关闭，1打开）
...
function MainMenuBar_PreLoad()
	...
	--变身、
	this:RegisterEvent("SKILL_UPDATE",true)									--停闪组队按钮
	this:RegisterEvent("SWITCHMENPAI",true)									--停闪组队按钮
	...
	this:RegisterEvent("CHAT_ACCEPT_DOUBLE_ACTION_STRING");		-- 从聊天栏里输入双人休闲动作的响应	
	this:RegisterEvent("UNINSTALL_CHAT_ACTION_BAR");					-- 卸载主菜单上的双人聊天动作按钮
	this:RegisterEvent("CLEAR_CHAT_ACTION_BAR");							-- 清理主菜单上过期的双人聊天动作按钮
	this:RegisterEvent("UNFLASH_TEAM_BUTTON",true)									--停闪组队按钮
...
function MainMenuBar_OnEvent(event)
	...
	-- 卸载双人休闲动作包时，同时删除拖动到MainMenuBar上的按钮信息	
	...
	elseif event == "UNFLASH_TEAM_BUTTON" then  --停闪组队按钮
		MainMenuBar_Stop_Flash_Team_Button()
...
function MainMenuBar_TextAccepted()
	...
	-- 预先判断输入字符串是不是有效的双人聊天动作
	local bChatAction = Talk : IsValidChatActionString(g_CurChannel, txt);
	
	-- 有效的双人聊天动作
	if (bChatAction == 1) then
		-- 询问对方是否可以做动作（暂不发送聊天信息字符串，也不记录历史聊天记录）
		g_ChatActionTxt = txt							-- 先把聊天动作字符串记录下来
		MainMenuBar_SetEditBoxTxt("")			-- 清空编辑框
		Talk : CanDoDoubleAction_Bar()
	
	-- 无效的双人聊天动作
	elseif (bChatAction == 2) then
		-- 不发送聊天信息字符串，也不记录历史聊天记录，只弹提示信息
		g_ChatActionTxt = ""							-- 清空聊天动作字符串
		MainMenuBar_SetEditBoxTxt("")			-- 清空编辑框
		  ...
		  if (tmpItem ~= -1) then
		  	...
		  	for j = 1, actionCount do
					...
					if (theAction:GetID() ~= 0) and (theAction:GetID() == tmpItem) then
						MAINMENUBAR_BUTTONS[i] : SetActionItem(-1);						-- 取消按钮上的ActionItem
						DataPool : UnInstall_RMB_ChatAction_BarItem(i-1);			-- 删除MainMenuBar中保存的ActionItem对应记录（编号对应DragName）
					end
				end
		  end
		end
...
function MainmenuBar_ShowTransfigureSkill( flag )
	...
	if flag == 1 then
		...
		if canbeCancel == 1 then
			if count >= 2 then --肯定需要大于2，因为有一个取消变身BUFF的技能
				local lastAction = count
				count = count - 1--最后一个固定在MAINMENUBAR_BUTTONS[60]位置，为取消变身技能action
				if(count > 9)then
					count = 9
				end
				local idx = 1;
		...
		--		local nSelfZoneWorldID = DataPool:GetSelfZoneWorldID();
			...
			--				-- 龙门特写 [聚宝盆:变身矿工BUFF的取消变身技能放到F10的位置]
...
function MainmenuBar_ShowSecWeaponfigureSkill( flag )
	if flag == 1 then
		...
		if canbeCancel == 1 then
			if count >= 2 then --肯定需要大于2，因为有一个取消变身的技能
				local lastAction = count
				count = count - 1--最后一个固定在MAINMENUBAR_BUTTONS[60]位置，为取消变身技能action
				if(count > 9)then
					count = 9
...
function NotifyDragDropDragged(cSourceName, nSourceIndex, cTargetType, nTargetIndex)
	-- PushDebugMessage("真元 "..cSourceName.." "..nSourceIndex.." "..cTargetType.." "..nTargetIndex)
end
