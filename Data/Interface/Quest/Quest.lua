...
function Quest_OnEvent(event)
	...
	elseif (event == "TOGLE_SKILLSTUDY") then
		AxTrace(0,0,"M· giao di®n H÷c, ðóng khung ð¯i thoÕi v¾i NPC");
		Quest_Close();
	...
	elseif (event == "TOGLE_BANK") then
		AxTrace(0,0,"M· giao di®n Ti«n Trang, ðóng khung ð¯i thoÕi v¾i NPC");
		Quest_Close();
		...
		elseif(strType == "id") then
			...
			if( tonumber( strScriptId ) == 808007 ) then
				...
				elseif( strTemp == "Ta mu¯n m· khóa ð½n lë" ) then
					nTitleType = 1
				elseif( strTemp == "Khóa t¤t cä" ) then
					nTitleType = 2
				elseif( strTemp == "Khóa ð½n lë" ) then
					nTitleType = 2
				elseif( strTemp == "Ð°ng ý" ) then
					nTitleType = 2
				end
			end
	...
	for i=1, nBonusNum do
		...
		if(strType == "money" and nNum > 0) then
			if(nRadio_Necessary == 1) then
				QuestGreeting_Desc:AddTextElement("#GThß·ng: #W")
				nRadio_Necessary = 0;
			end
...
--				AxTrace(1,1,"ActionID ="..ActionID);
		...
		elseif(strType == "itemradio") then
			if (nRadio == 1) then
				nRadio = 0;
				QuestGreeting_Desc:AddTextElement("#YHoàn thành nhi®m vø ch÷n thß·ng: #W");
			end
--			QuestGreeting_Desc:AddItemElement(nItemID, nNum, 0);
			nItemID = LifeAbility : GetQuestUI_Reward(i-1);
--			AxTrace(1,1,"nItemID="..nItemID .." i-1=".. i-1);
...
--	Quest_Frame_Debug:SetText("QUEST_STATE_MISSON_INFO");--get npcµÄname
	...
	Quest_Button_Continue:SetText("Tiªp");--¼ÌÐø
	Quest_Button_Refuse:SetText("Hüy");--È¡Ïû
end
...
--	Quest_Frame_Debug:SetText("AFTER_CONTINUE");--get npc name
	...
	Quest_Button_Accept:SetProperty( "Flash", "0" );
	Quest_Button_Refuse:SetText("Hüy");--È¡Ïû
	Quest_Button_Continue:SetText("Xong");--Íê³É
...
--				AxTrace(1,1,"ActionID ="..ActionID);
		...
		elseif(strType == "itemradio") then
			...
			if (nRadio == 1) then
				nRadio = 0;
				if nRadio_Necessary == 1 then
					QuestGreeting_Desc:AddTextElement("#YCh÷n 1 ph¥n thß·ng: #W");
				else
					QuestGreeting_Desc:AddTextElement("#YCh÷n 1 ph¥n thß·ng: #W");
				end
	...
	elseif( g_nQuestState == QUEST_STATE_AFTER_CONTINUE )then
		...
		if(bBeingRadio == 1) then
			if(bRadioSelect == 1)then
				...
				PushDebugMessage("Ch÷n 1 ph¥n thß·ng");
			end
		else
				QuestFrameMissionComplete(g_nRewardItemID);
				bBeingRadio = 0;
...
function Quest_HaoRen_Help(nIndex)
	...
	elseif nIndex == "40" then
		QuestGreeting_Desc:AddTextElement( "#Yv? Võ Cänh#r#r #WÐang vai di­n c¤p b§c ðÕt t¾i#G85#WC¤p Th¶i, có th¬ Ði¬m Kích Chü m£t biên cái nút: #GVõ Cänh L® Ð°#Wtiªn hành Võ Cänh dçn ðß¶ng nhi®m vø, hoàn thành nhi®m vø H§u có th¬ m· ra Võ Cänh công nång. #r#r#cfabf8fv« Võ Cänh c¤p b§c#r#W Võ