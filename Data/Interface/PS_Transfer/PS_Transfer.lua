...
function PS_Transfer_PreLoad()
	...
	this:RegisterEvent("BUYPSSUCCESS_CLOSE_SHOPLIST") --购买商店成功后关闭商店列表界面
...
function PS_Transfer_OnEvent(event)
	...
	elseif ( event == "PS_OTHER_TRANS_SELECT" )   then
		--PS_Transfer_UpdateFrame();
	elseif event == "BUYPSSUCCESS_CLOSE_SHOPLIST" then --购买商店成功后关闭商店列表界面
		--隐藏界面		
		this:Hide();		
...
function PS_Transfer_UpdateFrame()
	...
	if (tonumber(shopIndex) <= 0) then
		PS_Transfer_DPID_Text:SetText("店铺ID:")
	else
		PS_Transfer_DPID_Text:SetText("店铺ID:" .. shopIndex)
	end

	--店主ID
	local szID = PlayerShop:GetShopInfo("other","ownerid");
	PS_Transfer_ID:SetText("店主ID: ".. szID);
