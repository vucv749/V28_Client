...
function DoublePlay_Game_OnEvent(event)
	...
	if ( event == "UI_COMMAND" and tonumber(arg0) == 99829701) then		
		...
		local GameType = Get_XParam_INT(2) --游戏类型
		local Round   = Get_XParam_INT(3) --轮次
		local MyRTData  = Get_XParam_INT(4) --我的排名
		local MyScore = Get_XParam_INT(5) --我的分数
		local PicData = Get_XParam_INT(6) --PIC信息
		local nReOpen = Get_XParam_INT(7) --NULL Game2特有
		-----------下面界面数据---------------
		local RankData1  = Get_XParam_INT(8) --队伍排名
		local RankData2  = Get_XParam_INT(9) --队伍名字
		local TeamData1 = Get_XParam_INT(10) --队伍详细信息1
		local TeamData2 = Get_XParam_INT(11) --队伍详细信息2
		local TeamData3 = Get_XParam_INT(12) --队伍详细信息3
		local TeamData4 = Get_XParam_INT(13) --队伍详细信息4
		local TeamData5 = Get_XParam_INT(14) --队伍详细信息5
...
function DoublePlay_Game_SetData(GameType,Round,MyRank,MyScore,PicData,nRight,nReOpen)
	...
	--万位
	local PicData4 = math.floor(math.mod(PicData/10000,10)) --运算符号 1：+ 2：x

	--游戏类型
	if GameType == 1 then
		--个位 十位/玩家所踩数字，百位 千位/随机的数字，万位/运算符号
	...
	elseif GameType == 2 then
		...
		DoublePlay_Game_Game2Image1_2:Show() --set:DoubleGame01 image:DoubleGame_BKHover 获得
		DoublePlay_Game_Game2Image2_2:Show() --set:DoubleGame01 image:DoubleGame_BKNormal 获得
	...
	DoublePlay_Game_GameRound[GameType]:SetText(ScriptGlobal_Format("#{SRWF_230329_52}",Round)) --轮次
	DoublePlay_Game_GameRank[GameType]:SetText("当前排名:"..MyRank) --排名
	...
	DoublePlay_Game_GameNum[GameType]:SetText(ScriptGlobal_Format("#{SRWF_230329_54}", MyScore)) --我的队伍：%s0分
...
function DoublePlay_Game_SetTeamData(GameType,TeamNumData,TeamRankData,TeamData1,TeamData2,TeamData3,TeamData4,TeamData5,MyTeam)
	...
	for i = 1,5 do
		...
		if EachTeam[i][3] ~= 0 then
			if GameType == 3 then
				DoublePlay_Game_RankText4[i]:SetText("存活")
			else
				DoublePlay_Game_RankText4[i]:SetText("完成")
			end
		else
			if GameType == 3 then
				DoublePlay_Game_RankText4[i]:SetText("死亡")
			else
				DoublePlay_Game_RankText4[i]:SetText("未完成")
			end
		end
	end
